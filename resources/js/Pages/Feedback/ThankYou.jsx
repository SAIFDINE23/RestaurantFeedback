...
export default function ThankYou({ rating, googleUrl, company, reviewPlatforms }) {
    ...
    // Préparer la liste des plateformes activées
    const platforms = [
        {
            id: 'google',
            name: 'Google',
    ...
    // Filtrer les plateformes activées avec URL
    const enabledPlatforms = platforms.filter(platform => 
        reviewPlatforms?.[platform.id]?.enabled && reviewPlatforms?.[platform.id]?.url
    );

    const shouldShowReviewLinks = rating >= 4 && enabledPlatforms.length > 0;
                ...
                <div className="relative group">
                    ...
                    <div className="relative bg-white/90 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/50 overflow-hidden">
                        ...
                        <div className="relative bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 px-8 py-12 text-center">
                            ...
                            <p className="text-lg text-blue-50 font-medium max-w-md mx-auto">
                        ...
                        <div className="p-8 md:p-12">
                            ...
                            {/* Call to action Plateformes multiples */}
                            {shouldShowReviewLinks && (
                                <div className="bg-gradient-to-br from-gray-50 to-white rounded-2xl p-8 border border-gray-200 mb-8">
                                    ...
                                    <div className={`grid gap-4 ${enabledPlatforms.length === 1 ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'}`}>
                                        {enabledPlatforms.map((platform) => (
                                            <a
                                                key={platform.id}
                                                href={reviewPlatforms[platform.id].url}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="group relative inline-flex items-center justify-between gap-3 px-6 py-4 overflow-hidden rounded-xl transition-all duration-300 hover:shadow-lg"
                                            >
                                                <div className={`absolute inset-0 bg-gradient-to-r ${platform.color}`}></div>
                                                ...
                                                <ExternalLink className="relative w-5 h-5 text-white group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform flex-shrink-0" />
                                            </a>
                                        ))}
                                    </div>
